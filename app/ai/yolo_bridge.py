#!/usr/bin/env python3
"""ai/yolo_bridge.py — jembatan organizer -> model YOLO di app/ai/yolo/.
...
def project_root():
    """Lokasi folder YOLO (app/ai/yolo). Bisa dioverride via env AIORG_YOLO_PROJECT."""
    env = os.environ.get("AIORG_YOLO_PROJECT", "").strip()
    if env and os.path.isdir(env):
        return os.path.abspath(env)
    here = os.path.dirname(os.path.abspath(__file__))  # .../app/ai
    app_dir = os.path.dirname(here)                     # .../app
    yolo = os.path.join(app_dir, "ai", "yolo")          # .../app/ai/yolo
    if os.path.isdir(yolo):
        return os.path.abspath(yolo)
    return ""


def _best_bundled(root):
    """Bobot bawaan hasil blend (app/ai/yolo/best.pt)."""
    cand = os.path.join(root, "best.pt")
...
def _load_config():
    ...
    # Bobot bawaan blend diutamakan di atas path lama config.
    bundled = _best_bundled(root)
    if bundled:
        info["MODEL"] = bundled
    return info, root
...
def is_available():
    ...
    if not root:
        return False, {"reason": "folder app/ai/yolo tidak ditemukan"}
    try:
        import importlib.util
        if importlib.util.find_spec("ultralytics") is None:
            info["reason"] = ("ultralytics belum install "
    ...
    if not model or not os.path.isfile(model):
        info["reason"] = (f"model tidak ada: {model} "
                          "(latih dulu dari tab AI + Training)")
        return False, info
    return True, info
